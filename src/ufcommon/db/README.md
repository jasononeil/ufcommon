...
BelongsTo<A> specifies a simple, one way relation.  This model, belongs to another one.  A Purchase might belong to a Customer, a Photo might belong to a User etc.  
...
HasOne<A> and BelongsTo<B> are quite similar, and are related in many ways.  The key difference is that the foreign key is stored in the class with the BelongsTo field.  Let's look at an example.
...
ManyToMany<A,B> is used for situations where many things go together.  Each Student has many Classes, and each Class has many Students.  Each Tag has many Posts, and each Post has many Tags.  Defining a ManyToMany is simple:
    ...
    // And the related one, in your SchoolClass model:
    public var students:ManyToMany<SchoolClass, Student>;

The first type parameter should be the type of the current class/model, and the second is for the related class/model.  These are both fed into a ManyToMany object.  The behaviour is a little bit complicated, but it's sort of like this:
