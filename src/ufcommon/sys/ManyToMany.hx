...
{
	static var managers:Hash<CustomManager> = new Hash();
	...
	public function new(aObject:A, bClass:Class<B>)
	{
		...
		{
			manager = new CustomManager<A,B>(a,b,tableName);
	...
	function generateTableName(a, b)
	{
		...
		var aName = Type.getClassName(a).split('.').pop().toLowerCase();
		var bName = Type.getClassName(b).split('.').pop().toLowerCase();
