...
class ApiMacros
{
		...
		for (f in fields.copy())
		{
			switch (f.kind)
			{
				case FVar(TPath(p), _):
					...
					var instExpr = macro $i{f.name} = $newExpr;
	...
	static var classPos = Context.currentPos();
	static var localClass = Context.getLocalClass().get();
	static var fields = Context.getBuildFields();
